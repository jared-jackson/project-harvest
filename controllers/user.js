...
function generateToken(user) {
    var payload = {
        iss: 'my.domain.com', //TODO change to my domain
        sub: user.id,
        iat: moment().unix(),
...
exports.ensureAuthenticated = function (req, res, next) {
    if (req.isAuthenticated() && req.user.authorized) {
        next();
    } else {
...
exports.loginPost = function (req, res, next) {
    ...
    User.findOne({email: req.body.email, authorized: true}, function (err, user) {
        if (!user) {
            return res.status(401).send({
                msg: 'The email address ' + req.body.email + ' either does not exist or has not been admin approved.'
            });
        }
